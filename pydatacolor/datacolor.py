...
class DataColor:
    ...
    def interrupt_get(
        self, cmd: bytes, timeout=100, expect_read_num=1, timeout_reads=1000
    ):
    ...
    def measure_report(self):
        ...
        if len(result) == 13:
            ...
            self._print(
                f"Measurement  {data[0]:02x} B1:{data[1]} B2:{data[2]} G1:{data[3]} G2:{data[4]} O:{data[5]} R:{data[6]}"
            )
            # s = 4294967295.0
            s = 65535.0
    ...
    def measure_array_n(self, num_repeats):
        ...
        result = [
            np.average(summary[1]),
            np.average(summary[2]),
            np.average(summary[3]),
        ]
        return result
    ...
    def check_result(self, r, expected_result, comment, timeout=100):
        if list(r) != expected_result:
            self._print(
                f"Datacolour checking {cmd} got {r} expected {expected_result} : {comment}"
            )
