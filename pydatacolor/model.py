...
from lofigui import areset,aprint
...
class DataColorModel:
    ...
    def connect(self):
        ...
        if self.state == DataColorState.Disconnected:
            ...
            areset()
            aprint(f"Test colourimeter")
            self.datacolor = DataColor(verbose=False)
            self.datacolor._print = aprint
            if self.datacolor.dev:
                aprint(f"Reset colourimeter")
                self.datacolor.reset()
                aprint(f"Drain colourimeter")
                self.datacolor.drain(verbose=False)
                aprint(f"Serial number = {self.datacolor.get_serial_number()}")
                aprint(f"Drain colourimeter")
                self.state = DataColorState.Connected
            else:
                aprint(f"No colourimeter found")
